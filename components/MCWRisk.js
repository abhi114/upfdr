...
import {MCWownRisk as data} from './data';
...
const MCWRisk = () => {
  ...
  const DownloadExcel = async () => {
    ...
    console.log(
      ' main path is ' + RNFS.DownloadDirectoryPath + '/MCWownRisk.xlsx',
    );
    ...
    RNFS.writeFile(
      RNFS.DownloadDirectoryPath + `/MCWownRisk_${timestamp}.xlsx`,
      binaryStr,
      'ascii',
    )
      .then(() => {
  ...
  const generateCSV = async () => {
    ...
    try {
      ...
      const filePath = `${RNFS.DownloadDirectoryPath}/MCWownRisk_${timestamp}.csv`;
  ...
  const generatePDF = async () => {
    ...
    const timestamp = new Date().getTime();
    const destPath = `${RNFS.DownloadDirectoryPath}/MCWownRisk_${timestamp}.pdf`;
    try {
  ...
  return (
    <View style={styles.container}>
      <View style={{flexDirection: 'row', marginBottom: 6}}>
        ...
        <Text style={{color: '#FFFFFF'}}>/ MCW Own Risk</Text>
      </View>
      <Text style={styles.title}>MCW Own Risk List</Text>
      <TextInput
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <View style={styles.card}>
            ...
            <View style={styles.row}>
              <Text style={styles.cellTitle}>Declaration:</Text>
              <TouchableOpacity
                style={{
                  flex: 0.7,
                  borderRadius: 5,
                ...
                onPress={() => {}}>
                <Text style={[styles.cell, {color: '#90ee90'}]}>Download</Text>
              </TouchableOpacity>
            </View>
            <View style={styles.row}>
              <Text style={styles.cellTitle}>Approval Status:</Text>
              <TouchableOpacity
                style={{
                  flex: 0.7,
                  borderRadius: 5,
                  alignSelf: 'center',
                  borderColor:
                    item['Approval Status'] === 'Recommended'
                      ? '#90ee90'
                      : '#ff0000',
                  borderWidth: 2,
                  alignContent: 'center',
