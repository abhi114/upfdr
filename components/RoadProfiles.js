...
import {RoadProfiles as data} from './data';
...
import DarkThemeFlatList from './Helpers/CardComponent';
import EnhancedDarkThemeFlatList from './Helpers/CardComponent';
...
const RoadProfiles = () => {
  ...
  const DownloadExcel = async () => {
    ...
    console.log(
      ' main path is ' + RNFS.DownloadDirectoryPath + '/RoadProfiles.xlsx',
    );
    ...
    RNFS.writeFile(
      RNFS.DownloadDirectoryPath + `/RoadProfiles_${timestamp}.xlsx`,
      binaryStr,
      'ascii',
    )
      .then(() => {
  ...
  const generateCSV = async () => {
    ...
    try {
      ...
      const filePath = `${RNFS.DownloadDirectoryPath}/RoadProfiles_${timestamp}.csv`;
  ...
  const generatePDF = async () => {
    ...
    let options = {
      html: htmlContent,
      fileName: 'RoadProfiles',
      directory: 'Documents',
    };

    let file = await RNHTMLtoPDF.convert(options);
    const timestamp = new Date().getTime();
    const destPath = `${RNFS.DownloadDirectoryPath}/RoadProfiles_${timestamp}.pdf`;
    try {
    ...
    <ScrollView contentContainerStyle={styles.scrollContainer}>
      <View style={styles.container}>
        <View style={{flexDirection: 'row', marginBottom: 6}}>
          ...
          <Text style={{color: '#FFFFFF'}}>/ Road Profile</Text>
        </View>
        <View style={styles.card}>
          <Text
            style={{color: '#FFFFFF', fontWeight: 'bold', marginBottom: 10}}>
            Total Road Profile Uploaded
          </Text>
          <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
            <View>
              ...
              <Text style={{color: '#aaaaaa', fontSize: 12}}>
                Total Number of Road Profile Uploaded
              </Text>
            </View>
            <View style={{margin: 2}}>
              <Icon name="gear" size={30} color={'#0000FF'} />
            </View>
          </View>
        </View>
        <View style={{flexDirection: 'row', marginBottom: 5}}>
          <View
            ...
            <Text
              style={{color: '#FFFFFF', fontWeight: 'bold', marginBottom: 10}}>
              Roads with Road Profile Uploaded
            </Text>
            <View
              ...
              <View>
                ...
                <Text style={{color: '#aaaaaa', fontSize: 12}}>
                  Road Profile Uploaded
                </Text>
              </View>
              <View style={{margin: 2}}>
                <Icon name="gear" size={30} color={'#FFFF00'} />
              </View>
            </View>
          </View>
          <View
            ...
            <Text
              style={{color: '#FFFFFF', fontWeight: 'bold', marginBottom: 10}}>
              Roads With Pending Road Profile
            </Text>
            <View
              style={{flexDirection: 'row', justifyContent: 'space-between'}}>
              <View>
                ...
                <Text style={{color: '#aaaaaa', fontSize: 12}}>
                  Number of Roads With pending Road Profile
                </Text>
              </View>
              <View style={{margin: 1}}>
                <Icon name="gear" size={30} color={'#ff0000'} />
              </View>
            </View>
          </View>
        </View>
          ...
          renderItem={({item}) => (
            <View style={styles.card}>
              ...
              <View style={styles.row}>
                ...
                <Text style={styles.cell}>{item.contractor}</Text>
              </View>
              <View style={styles.row}>
                ...
                <TouchableOpacity
                  ...
                  <Text style={[styles.cell, {color: '#0090E7'}]}>View</Text>
                </TouchableOpacity>
              </View>
            </View>
          )}
